...
{	
	...
	protected function CekKoorProdi($id_prodi){
		$Data = DB::table('a_tanda_tangan')->select('a_tanda_tangan.id')->where([['kode_prodi','=',$id_prodi],['status','=','1']])->first();
	...
	public function tambahsuratundangan(Request $request){
		...
		$valKoor = $this->CekKoorProdi($request->id_prodi);
		...
		$result = 	DB::table('a_srt_udg_penguji')
	    			->insert(
	        			[	'no_surat' => sprintf("%03s", $rtz + 1).$this->nosurattugas(), 
	        				...
	        				'keterangan_nilai' => $request->jenis_nilai,
	        				'kelompok_penilaian_proposal' => '2',

	        				'id_tanda_tangan' => $valKoor,
